...
class MainViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    MainViewHolder(@NonNull ViewGroup parent, OnViewHolderListener listener) {
        super(LayoutInflater.from(parent.getContext()).inflate(
                R.layout.item_main_item, parent, false));
        mListener = listener;
        mVName = (TextView) itemView;
        itemView.setOnClickListener(this);
    }

    void bind(int position, MainDataAdapter adapter) {
        mItem = adapter.getItem(position);
        mVName.setText(adapter.getItemName(mItem));
    }
